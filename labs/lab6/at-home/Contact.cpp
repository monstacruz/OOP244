...
namespace sict{
    ...
    Contact::Contact(Contact & input){
        set(input.name, input.numbers, input.size);
    }
    
    Contact Contact::operator = (const Contact & input){
        if (this != &input)
            set(input.name, input.numbers, input.size);
        return *this;
    }
    
    Contact Contact::operator += (long long int input){
        bool valid = false;
        if (input > 10000000000 && input < 999999999999){
            valid = true;
        }
        if(valid){
            long long int newNumbers[size + 1];
            for (int i = 0; i < size; i++){
                newNumbers[i] = numbers[i];
            }
            size++;
            newNumbers[size-1] = input;
            numbers = new long long int [size];
            for (int i = 0; i < size; i ++){
