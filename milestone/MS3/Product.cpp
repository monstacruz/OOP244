...
// OPERATORS
    ...
    bool Product::operator > (const Product & input) const{
        bool result = false;
        if (strcmp(pSku, input.pSku) > 0)
            result = true;
        return result;
    }
...
// OSTREAM & ISTREAM
    ...
    ostream& operator << (ostream& os , const Product& input){
        input.write(os, false);
        return os;
    }
    
    istream& operator >>(istream& is, Product& input){
        input.read(is);
        return is;
    }
...
// HELPER FUNCTIONS
    double operator +=(double & doubleInput, const Product& prodInput){
